...
class ArticleScaleBenchmarkTests(unittest.TestCase):
    ...
    def test_small_benchmark_supports_guarded_auto_backend(self) -> None:
        ...
        self.assertIn("article_page_default_first_screen", summary["rollout_guard"]["page_read_timings_ms"])
        self.assertIn("bulk_upsert_small_batch", summary["rollout_guard"]["crud_timings_ms"])
        self.assertIn("warm", summary["rollout_guard"]["refresh_timings_seconds"])
        operations = {operation["name"]: operation for operation in summary["operations"]}
