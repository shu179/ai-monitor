...
def test_unknown_legacy_modes_fall_back_to_browser(isolated_task_state):
    ...
    with (
        ...
        patch(
            ...
            return_value={"attempted": True, "success": True, "found_results": 5, "error_message": ""},
        ),
    ):
    ...
    assert calls == [
        ...
        "browser:kimi:品牌B 旧模式问答",
        "browser:tongyi:品牌B 旧模式问答",
        "browser:yuanbao:品牌C 旧模式问答",
    ...
    assert [(item["keyword"], item["platform"], item["mode"]) for item in results] == [
        ...
        ("品牌B 旧模式问答", "kimi", "browser"),
        ("品牌B 旧模式问答", "tongyi", "browser"),
        ("品牌C 旧模式问答", "yuanbao", "browser"),
    ...
    assert report["mode"] == "browser"
    assert report["attempted_queries"] == 6
    assert report["success_queries"] == 5
    ...
    assert [item["platform"] for item in diagnostics] == ["deepseek"]
...
def test_missing_screenshot_keeps_notification_fallback_explainable(tmp_path):
    ...
    keywords = [
        ...
        {"keyword": "品牌A Kimi", "brand": "品牌A", "platforms": ["kimi"], "mode": "browser"},
    ]
    all_results = [
        {
            "keyword": "品牌A 豆包",
...
def test_structural_and_temporary_failure_report_fields_are_stable(isolated_task_state):
    ...
    class _FailingBrowserPlatform(_ReplayBrowserPlatform):
        def search(self, keyword: str, brand: str) -> tuple[int, str]:
            self._calls.append(f"browser:{self.platform_name}:{keyword}")
            self.last_error = "未配置有效 webhook" if self.platform_name == "tongyi" else "请求超时"
            raise RuntimeError(self.last_error)

    def create_platform(platform_name: str, *, config=None, inspect=False, stop_checker=None):
        del config, inspect, stop_checker
        return _FailingBrowserPlatform(platform_name, isolated_task_state / "screenshots", [])
...
def test_missing_rank_found_mentioned_and_screenshot_fields_default_to_no_hit(isolated_task_state):
    ...
    class _NoHitBrowserPlatform(_ReplayBrowserPlatform):
        def search(self, keyword: str, brand: str) -> tuple[int, str]:
            self._calls.append(f"browser:{self.platform_name}:{keyword}")
            self.last_answer_text = "没有命中品牌"
            return 99, ""

    def create_platform(platform_name: str, *, config=None, inspect=False, stop_checker=None):
        del config, inspect, stop_checker
        return _NoHitBrowserPlatform(platform_name, isolated_task_state / "screenshots", [])
