...
class CloudPlatformAutoSync:
        ...
        try:
            while not self._stop_event.is_set():
                ...
                if first_sync_for_login or now - self._last_pull_started_at >= self._pull_interval_seconds:
                    ...
                    try:
                        pull_result = self._invoke_pull_tasks()
                    except Exception as exc:
                    ...
                    if pull_result.get("ok"):
                        self._update_status(
                            ...
                            last_pull_metrics=_pull_metrics(pull_result),
                        )
                        self._clear_error()
                    else:
                        self._update_status(last_pull_metrics=_pull_metrics(pull_result))
    ...
    def _pull_now_from_event(self, event_name: str) -> None:
        force_task_pull = event_name in FULL_TASK_PULL_EVENT_NAMES
        pull_result = self._invoke_pull_tasks(force=force_task_pull)
