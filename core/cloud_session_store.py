...
class CloudSessionStore:
    ...
    def save_login(self, *, base_url: str, token_pair: dict[str, Any]) -> dict[str, Any]:
        payload = self._build_login_payload(base_url=base_url, token_pair=token_pair)
        return self.save(payload)

    def update_user(self, user: dict[str, Any]) -> dict[str, Any]:
        with self._lock:
            session = self._load_unlocked()
            if not session:
                return {}
        ...
        *,
        ...
        with self._refresh_lock:
            with self._lock:
                ...
                payload = self._build_login_payload(base_url=normalized_base_url, token_pair=token_pair)
                payload["saved_at"] = local_now().isoformat(timespec="seconds")
                self._save_unlocked(payload)
                return payload
    ...
    def _build_login_payload(self, *, base_url: str, token_pair: dict[str, Any]) -> dict[str, Any]:
        return {
